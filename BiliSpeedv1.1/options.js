...
function renderSpeedList() {
  ...
  speedList.innerHTML = sortedSpeeds.map(speed => `
    <div class="speed-item">
      <span class="speed-value">${Number(speed.toFixed(2))}x</span>
...
function addCustomSpeed() {
  ...
  showMessage(`已添加倍速 ${Number(speed.toFixed(2))}x`, 'success');
}
...
function removeCustomSpeed(speed) {
  ...
  updateUI(); // 立即更新UI
  saveSettings();
  showMessage(`已删除倍速 ${Number(speed.toFixed(2))}x`, 'success');
}
