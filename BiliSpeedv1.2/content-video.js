...
(function() {
  ...
  function tryFindVideoAndMenu() {
    ...
    // 查找倍速菜单。仅在菜单/列表是新节点或视频元素变化时才重新注入，
    // 避免高频扫描（弹幕等）反复重建菜单项
    const rateBtn = document.querySelector('.bpx-player-ctrl-playbackrate');
    if (rateBtn) {
      const rateList = rateBtn.querySelector('ul');
      if (rateList && (videoChanged || rateBtn !== playbackRateMenu || rateList !== playbackRateList)) {
        playbackRateMenu = rateBtn;
        playbackRateList = rateList;
        injectCustomSpeeds();
      }
    }
  }

  // rAF 节流的扫描调度：B站页面DOM变化极高频（弹幕、动画等），
  // observer 回调只做标记，把实际扫描合并为每帧最多一次，减少重复查询开销
  ...
  // 倍速显示格式与B站原生菜单一致：至少保留一位小数（3 → 3.0x，3.2 → 3.2x，3.25 → 3.25x）
  ...
  function injectCustomSpeeds() {
    ...
    for (const speed of sortedCustomSpeeds) {
      ...
      li.textContent = formatSpeedLabel(speed);
  ...
  // 正在输入框/富文本中打字时不触发快捷键。
  // B站评论区是 Shadow DOM 组件（bili-comment-rich-textarea 内的 div.brt-editor），
  // 事件冒泡到 document 时 target 会被重定向为 shadow host（自定义元素标签，
  // isContentEditable 为 false），必须遍历 composedPath() 才能识别其中的真实输入元素，
  // 否则评论框内按方向键会被劫持为快进、打字会被快捷键吞掉。
  function isTypingEvent(event) {
    const path = (typeof event.composedPath === 'function') ? event.composedPath() : [];
    for (const node of path) {
      if (isEditableNode(node)) return true;
      if (node === document) break; // 之后只剩 html/window，不会再有输入元素
    }
    // 兜底：焦点元素本身（或其所在 open shadow 内的焦点元素）可编辑
  ...
  function showRateToast(sticky = false) {
    ...
    if (!rateToast || !container.contains(rateToast)) {
      ...
      Object.assign(rateToast.style, {
        position: 'absolute',
        // 靠上放置，避开B站全屏时顶部的"已进入全屏模式"提示（具体高度待实测微调）
        top: '1.5%',
        left: '50%',
    ...
    rateToast.textContent = formatSpeedLabel(videoElement.playbackRate);
